...
const ViewList = ({
  ...
  const convertTimeto24 = (time: any) => {
    ...
    if (time !== '') {
      if (parseInt(response[0]) >= 12) return `${response[0]}:${response[1]} PM`
      else return `${response[0]}:${response[1]} AM`
    }
    return ''
  }
    ...
    <div className={styles.viewListConten}>
      ...
      {consult === 1 ? (
        <Select
          ...
          onClick={onClick}
        >
          {index === 1 ? <option key="0">-- Seleccione --</option> : null}
