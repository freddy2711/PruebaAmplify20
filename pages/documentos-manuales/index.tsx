...
const index = () => {
    ...
    const FormatedValueDocuments = (Data:any) => {
        ...
        for (const element of formated) {
            ...
            const urlText = urlformated.slice(0,urlpositionFormated + 12).replace("','xClase','height=600,width=800","").replace("','","")
            const blockdata = {
                    ...
                    <ul style={{color:'#007bff',fontSize:' 1.1em'}}>
                        {
                            DocumentData.map((x:any) => (
                                <li style={{cursor:'pointer'}} onClick={() => handleSelectDocument(x.url)} key={Math.random()}>{x.title}</li>
                            ))
                        }
