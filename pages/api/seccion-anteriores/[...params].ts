...
const handler = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    switch(params[0]){
      ...
      case 'SessionsByClass':{
        try {
          const URL = apiPath.seccionAnteriores.PATH_GetSessionsByClass(params[2],params[3],params[4])
          const { data } = await axiosfetchPrivate(URL)
      ...
      case 'PeriodPayment':{
        try {
          const URL = apiPath.seccionAnteriores.PATH_GetPeriodPayment(params[2],params[3])
          const { data } = await axiosfetchPrivate(URL)
