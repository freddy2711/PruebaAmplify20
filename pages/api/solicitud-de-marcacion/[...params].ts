  ...
  switch (params[0]) {
    ...
    case 'insertar': {
      ...
      try {
        ...
        // console.log('API_INSERTAR__',resp.data.errorMessage)

        if (resp.data.errorMessage) {
          throw new Error(`${resp.data.errorMessage}`)
        }

        if (resp.data.detail && parseInt(resp.data.detail) === 0) {
          // const error = new Error('No se pudo insertar el registro')
          throw new Error(
            'No se pudo insertar el registro duplicado, ya cuenta con una solicitud.'
          )
