...
const handler = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    switch (params[0]) {
        ...
        case 'TeacherNotesStudentTutorin': {
            const URL = apiPath.reporteTutorias.PATH_GetTeacherNotesStudentTutoring(params[1],params[2])
            try {
              const { data } = await axiosfetchPrivate(URL)
              const result = data.detail
              res.status(200).json(result)
        ...
        case 'StudentTutoring': {
            const URL = apiPath.reporteTutorias.PATH_GetNotesStudentTutoring(params[1])
            try {
              const { data } = await axiosfetchPrivate(URL)
              const result = data.detail
              res.status(200).json(result)
