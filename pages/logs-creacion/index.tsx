...
const TableDinamic = dynamic(() => import('../../components/UI/molecules/tableDinamic/Table'),
{
    ssr: false,
}
)
...
const LogsCreacion = () => {
  ...
  const SemesterCode = get(SET_SEMESTERCODE) === null ? '2022-1' : get(SET_SEMESTERCODE)
  ...
  const ApilistTokenActive = async (userCode:any,semesterCode:any,limitState:any) => {
    const response = await apiLogCreate.listTokenActive(userCode,semesterCode,limitState)
    return response
  }

  const ApilistTokenGenerat = async (userCode:any,semesterCode:any) => {
    const response = await apiLogCreate.listTokenGenerate(userCode,semesterCode)
    return response
  } 
...
const FormatedValueTokenActive = (value:any) => {
  if(value !== undefined){
    ...
    return `${moment(date).format('DD-MM-YYYY')}, ${convertTimeto24(moment(date).format('HH:mm'))}`
  }else{
    return "DD-MM-YYYY"
  }
}
...
const convertTimeto24 = (time:any) => {
  ...
  if(time !== ""){
      if(parseInt(response[0]) >= 12)
          return `${response[0]} pm`
      else
          return `${response[0]} am`
  }
  return ""
}
...
useEffect(() => {
  const Load = async () => {
    setloading(true)
    const TokenActiveData = await ApilistTokenActive(UserCode,SemesterCode,1)
    setlistTokenActive(TokenActiveData[0])
    const TokenGeneratData = await ApilistTokenGenerat(UserCode,SemesterCode)
    if(TokenGeneratData.length !== 0) FormatedTokenGeneratData(TokenGeneratData,setlistTokenGenerat)
    else ViewMessage(0)
    setloading(false)
  }
    ...
    <div className={styles.contenido}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.subtitle}>
          <span>Lista de Token Generados Semestre {listTokenActive?.semesterCode}</span>
        </div>
