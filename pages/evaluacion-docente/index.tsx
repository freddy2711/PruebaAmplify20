    ...
    const FormatedValue = (Data:any) => {
        ...
        for (const element of formated) {
            ...
            for (const item of filterdata) {
                ...
                const formatedTitle = item.slice(positionItem).trim().replace(`style="cursor:pointer;"`,"").replace(">","")
                ...
                const urlData = formatedURL.slice(0,urlpositionFormated).replace("','","")
                ...
                <div>
                 
                    {
                        DocumentData.map((x:any) => (
                            <div key={Math.random()}>
                                   <br />
                                <h6 key={Math.random()} className='text-dark font-weight-bold mb-1'>{x.Titletext}</h6>
                                {
                                    x.getItemsData.map((x:any) => (
                                        <div key={Math.random()}>
                                            <ul key={Math.random()} style={{color:'#007bff',fontSize:' 1em',margin:'0'}}>
                                                <li key={Math.random()} style={{cursor:'pointer'}} onClick={() => handleSelectDocument(x.url)} >{x.title}</li>
                                            </ul>
                                        </div>
                                    ))
                                }
