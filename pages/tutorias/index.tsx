...
const index = () => {
  ...
  const [TeacherNotesStudentTutori,setTeacherNotesStudentTutori] = useState<any>([])
  const [StudentTutoring,setStudentTutoring] = useState<any>([])
  const DUENOSESSION = get(SET_DUENO_SESSION) === null ? 'RVI' : get(SET_DUENO_SESSION)
  const USER = get(SET_TEACHERCODE) === null ? 'N00011885' : get(SET_TEACHERCODE)
  ...
  const ApiTeacherNotesStudentTutorin = async (clasCode:any,semCode:any) => {
    const response = await apiTutorias.TeacherNotesStudentTutorin(clasCode,semCode)
    return response
  }
  ...
  const formatedData = (obj: any, setstate: any) => {
    const rows = obj.map((item: any) => ({
      ...item,
      select: (
        <p style={{cursor:'pointer',color:'#D24726',fontWeight:'bold'}} onClick={() => handleClickRow(item)} className="text-decoration-none text-center w-100 d-block">
          seleccionar
        </p>
      ),
    }))
  ...
  const formatedDataTeacherNotesStudentTutorin = (obj: any, setstate: any) => {
    const rows = obj.map((item: any) => ({
      ...item,
      select: (
        <p style={{cursor:'pointer',color:'#D24726',fontWeight:'bold'}} onClick={() => handleClickRowTeacherNotesStudentTutorin(item)} className="text-decoration-none text-center w-100 d-block">
          seleccionar
        </p>
      ),
    }))
  ...
  const handleClickRow = async (item:any) => {
    setLoading(true)
    const response = await ApiTeacherNotesStudentTutorin(item.claCode,item.semCode)
    setTeacherNotesStudentTutori([])
      setStudentTutoring([])
    if(response.length <= 0 ) ViewMessage(0)
    else formatedDataTeacherNotesStudentTutorin(response,setTeacherNotesStudentTutori)
    setLoading(false)
  }
