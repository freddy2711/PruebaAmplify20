...
const Index = () => {
  ...
  const callApiLogin = async (codeTeacher: any, day: any) => {
    ...
    const semesterTemp = await apiLogin.TokenCoupling(codeTeacher)
    if (callErrorValid(semesterTemp, setloading) === undefined) return
    // const semesterCode = semester[0]?.semesterCode
    // const obj: any = {
    //   userCode: codeTeacher,
    //   semesterCode,
    //   fechaHora: dateNow,
    // }
    // if (couplingTemp === undefined) {
    //   const coupling = await apiLogin.PutLog(obj)
    //   if (callErrorValid(coupling, setloading) === undefined) return
  ...
  // const callApiLoginValid = async (codeUser: any) => {
  //   console.log('callApiLoginValid', codeUser)
