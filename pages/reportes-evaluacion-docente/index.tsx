...
const index = () => {
  ...
  const UserCode = get(SET_TEACHERCODE) === null ? 'N00011885' : get(SET_TEACHERCODE)

  

  const ApiResultTeacherEvaluation = async (UserCode:any) => {
    const response = await apiReportTeacherEvaluation.listResultTeacherEvaluation(UserCode)
    return response
  }
  ...
  const ApiDocumentAWSS3 = async (periodo:any,name:any) => {
    const response = await apiReportTeacherEvaluation.lsttDocumentsAWSS3(periodo,name)
    return response
  }

  const ApiDownloadAWSS3 = async (rutaUrl:any) => {
    const response = await apiReportTeacherEvaluation.DownloadDocumentsAWSS3(rutaUrl)
    return response
  }
  ...
  const AsignaValores = (periodo:string,tipo:number,strsede:string,dtresultados:[]) => {
    let urlTipo = "";
    let txt = "";
    let url = "";
    ...
    if(dtresultados.length > 0 ){
      const response:any = dtresultados.find((x:any) => x.sPeriodo === periodo && x.sedCode.toUpperCase() === strsede && x.sTipo === urlTipo)
  ...
  const ValidateListData = async (obj:any,sedeData:any) => {
    ...
    await Promise.all(periodos.map(async (periodo:any) => {
      ...
      const DataRow = await Promise.all(sedeData.map(async (x:any) => {
        ...
        if(datav1){
          ...
          const GetSede = sedeData.find((item:any) => item.codigoSede === sede) 
  ...
  const TargetConten = () => {
    return(
    ReportData.map((y:any) => {
        return(
        <div key={Math.random()} >
          <span style={{ background: '#6C757D', padding: '7px', color: 'white', fontWeight: 'bold', borderRadius: '5px' }}>{y.titulo}</span>
            <div className='row'>
            {
            y.filterrow.map((x: any) => { 
              return(
              <div key={Math.random()} className="col-sm-2" style={{paddingTop:'10px'}}>
              <p style={{color:'#17B6DD',fontWeight:'bold'}}>{x.sedeFormated}</p>
              <div>
                {
                  x.clase.url === "" ? (<p>{x.clase.txt}</p>) : (<p onClick={() => DownloadFile(x.clase.url)} style={{color:'red',cursor:'pointer'}}>{x.clase.txt}</p>)
                }
                {
                  x.curso.url === "" ? (<p>{x.curso.txt}</p>) : (<p onClick={() => DownloadFile(x.curso.url)} style={{color:'red',cursor:'pointer'}}>{x.curso.txt}</p>)
                }
              </div>
              </div>
              )
