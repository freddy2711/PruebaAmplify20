...
import { CLASEID_REPORTES, LST_SELECTED_COURSE, SET_DATA_DOCENTE } from "../../consts/storageConst"
...
const ReporteCompetencias = () => {
    ...
    const [lstCompetenceGeneralByClass,setlstCompetenceGeneralByClass] = useState<any>([])
    ...
    const ApiCompetenceGeneralByClass = async (classCode:any) => {
      const response = await apiReportesAcademicos.listCompetenceGeneralByClass(classCode);
      return response
    }
    ...
    const ApiCompetenceSchedule = async (classId:any,noteId:any) => {
      const response = await apiReportesAcademicos.listCompetenceSchedule(classId,noteId);
      return response
    }
    ...
    const formatedDataNotes = (objNotas: any,objCompetenceSchedule: any, setstate: any) => {
      ...
      const FormatedCompetenceSchedule = objCompetenceSchedule.map((item:any) => {
        item.notaPorcentual = item.notaPorcentual === "" ? "" : item.notaPorcentual
        item.notaVigesima = item.notaVigesima === "" ? "" : item.notaVigesima
        item.EstadoEvaluacion = item.notaPorcentual === "" ? "Pendiente" : "Completo"
        return item
      })
      
      rows.forEach((e:any,x:any) => {
          output.push({...e, notaPorcentual : FormatedCompetenceSchedule[x].notaPorcentual, notaVigesima : FormatedCompetenceSchedule[x].notaVigesima,EstadoEvaluacion : FormatedCompetenceSchedule[x].EstadoEvaluacion });
      });
    ...
    const handleSelectedChange = async (event: React.FormEvent<HTMLInputElement>) => {
      const response = (event.target as HTMLInputElement).value

      if(response !== "0"){
        setloading(true)
        const resultNotes = await ApiNotes(ClassCode)
        const resultCompetenceSchedule = await ApiCompetenceSchedule(ClassCode,response)
  ...
  const COLUMNS = [
    ...
    { label: 'Nombre del alumno', field: 'AluNameFull',key: 'AluNameFull', sort: 'asc' },
    ...
    { label: 'Porcentaje', field: 'notaPorcentual',key: 'notaPorcentual', sort: 'asc' },
    { label: 'Nota', field: 'notaVigesima',key: 'notaVigesima', sort: 'asc' },
    { label: 'Estado de Matricula', field: 'MadeEstado',key: 'MadeEstado', sort: 'asc' },
    { label: 'Estado de Evaluación', field: 'EstadoEvaluacion',key: 'EstadoEvaluacion', sort: 'asc' },
  ]
        ...
        <div className={styles.content}>
          ...
          <div className={styles.viewContenListBtns}>
            ...
            <div className={styles.viewListConten}>
                <div><p style={{margin:'0',whiteSpace:'nowrap'}}>Seleccionar Competencia:</p></div>
                <div>
                  <Select
                    id="formato"
                    classname="primary"
                  ...
                  >
                  {
                    lstCompetenceGeneralByClass.length > 0 ? (
                      lstCompetenceGeneralByClass.map((x:any) => (
                        <option key={x.CompetenceId} value={x.CompetenceId}>{x.CompetenceTitle}</option>
                      ))
                    ): (
...
ReporteCompetencias.title = 'Reporte de Evaluación de Competencias - Portal Docentes'
