...
import { faTimes ,faFileText,faFileZipper,faFileVideo,faPaperclip, faFileImage ,faFileAudio, faFile, faFilePdf, faFileWord, faFileExcel, faFilePowerpoint  } from '@fortawesome/free-solid-svg-icons'
// import Swal from 'sweetalert2'
...
const index = () => {
	...
	useEffect(() => {
		...
		const loadData = async () => {
			...
			try {
				const { data } = await apiSoporteVirtual.docenteConsulta(teacherCode,dataid)
				console.log(data)

				const resp = await apiSoporteVirtual.apiAnexos(teacherCode,data[0].asignedTo)
	...
	const insertarImagen = async (obj:any) =>{
		try {
			...
			const resp = await apiSoporteVirtual.insertImg(usuario, nameS3, name, extend, size, adviser)
	...
	const printAdjunto = (url: string, name: string, nameS3: string, tipo:string, AluCode: string,Adviser: string) =>{
    ...
    if (validFile(fileObj)) {
      ...
      console.log('NEWWWOBJJ__:', newObj)
			...
			try {
				...
				const tipo = fileObj.name.substring(fileObj.name.lastIndexOf('.') + 1, fileObj.name.length)
	...
	const removeImage = async (e:any, nameS3: string, AluCode: string,Adviser: string) =>{
		e.preventDefault()
		...
		try {
			...
			const respClean:any = await apiSoporteVirtual.cleanAnexo(AluCode,nameS3,Adviser)
	...
	const save = async (e:any) => {
		...
		try {
			...
			const { data } = await apiSoporteVirtual.requestConsulta("N00011885", descrip, dataid)
			const resp = data[0]
				...
				<div className={styles.newConsulta}>
					...
					<div className="chat">

						{
							...
							detalle.map((item: any, index:number) => (
								<div key={index} className={`form-group chat-one chat-one-${item.id}`}>
                  <div className='img-chat'>
                      <img src='https://intranet.upn.edu.pe/wsfoto/foto/MVdYR2dBdGdTbCsyd0NnaEdsY0NsUT09/Collaborator' />
                      <p style={{lineHeight:"1", fontSize:"11px", marginTop:"8px"}}>
                  ...
                  <div className='msg-chat'>
                      <div className='msg-chat-content'>
													...
													<div dangerouslySetInnerHTML={{__html: item.response.replace(/\\"|"/g, '')}}>
                                  ...
                                  <ul className="body-list-load-file-msg">
																		{
																			...
																			anexos.map((item:any, index: number) => (
																				<li key={index}>
																					...
																					<a href='#' style={{ cursor:'pointer'}} title="Descargar adjunto" target="_blank" rel="noreferrer" onClick={(e:any) => handleDownloadList(e, `${item.state}.${item.asignedTo}`)} >
																					<FontAwesomeIcon icon={convertFileIcon(item.responseArea)} className="fa-sm me-2"/>
																					{item.responseArea}
																					</a>
																				</li>
																			 )): null
							...
							<Fragment>
								<form>
									<div className='adjuntos'>
											<ul id='cphSite_body_list_load_file' className='body-list-load-file list-unstyled row"'>
													{	
														adjunto.map((item:any, index: number) => (
															<li key={index}>
																<a href='#' data-names3={item.nameS3} className='text-decoration-none' onClick={(e:any) => removeImage(e , `${item.nameS3}.${item.tipo}`,item.AluCode, item.Adviser)}>
																	<FontAwesomeIcon icon={faTimes} className="fa-sm"/>&nbsp;&nbsp;
																</a>
																<a href={item.url} target="_blank" rel="noreferrer">
																<FontAwesomeIcon icon={convertFileIcon(item.name)} className="fa-sm"/>
																&nbsp;&nbsp; {item.name}
																</a>
															</li>
														))
											...
											<div>
												<FileInputComponent
													...
													buttonComponent={
														...
														>
															<small><FontAwesomeIcon icon={faPaperclip} className="fa-sm"/></small>
															Adjuntar archivos <small>(max. 5MB por adjunto)</small>
														</label>
													}
													labelStyle={{
														fontSize: 16,
									...
									<br/>
									<QuillRichText refe={quillObj} value={descrip} set={setDescrip} handleImage={handleImage}/>
								</form>
							</Fragment>
						): null	
					}
