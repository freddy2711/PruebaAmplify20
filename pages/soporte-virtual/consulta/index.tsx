		...
		switch (id) {
			case 'ddlTipo': {
				...
				if(parseInt(value) !== 0){
					setloading(true)
					try {
						const { data } = await apiSoporteVirtual.tipocse(value,'SEL-SUBTIPO')
		...
		try {
			...
			await Promise.all(
				datos.data.map(async (item:any) => {
					...
					const respClean:any = await apiSoporteVirtual.cleanAnexo(AluCode,item.state,Adviser)
					console.log('respClean', respClean.data)
      ...
      <div className={styles.content}>
				...
				<div className={styles.newConsulta}>
					...
					<div>
						<div>
							<div> 
								<div className='form-group row mb-3'> 
									<label className='col-md-3 text-end'><b>tipo:</b></label>
							...
							<div>
								<div className='form-group row mb-3'>
									<label className='col-md-3 text-end'><b>Sub Tipo:</b></label> 
									<div className="col-md-6">
										<AsyncSelect
											options={subTipo}
											id="ddlSubTipo"
						...
						<label><b>Por favor describa su consulta:</b></label>
						<div>
								...
								<ul id='cphSite_body_list_load_file' className='body-list-load-file list-unstyled row"'>
										{	
											 adjunto.map((item:any, index: number) => (
												<li key={index}>
													<a href='#' className='text-decoration-none' data-names3={item.nameS3} onClick={(e:any) => removeImage(e , item.nameS3, item.AluCode, item.Adviser)}>
														<FontAwesomeIcon icon={faTimes} className="fa-sm"/>&nbsp;&nbsp;
													</a>
													<a href={item.url} target="_blank" rel="noreferrer">
													<FontAwesomeIcon icon={convertFileIcon(`${item.name}.${item.tipo}`)} className="fa-sm me-2"/>
													{item.name}
													</a>
												</li>
											 ))
