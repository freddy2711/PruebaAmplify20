...
import styles from '../../components/templates/recuperar/resumen/Resumen.module.scss'
...
const resumen = () => {
    ...
    const [TeacherCoursesSelected,setTeacherCoursesSelected] = useState<Selected>({ClaCodigo:'',ClaTipo:'',SemCodigo:'',SedCodigo:'',ClaMetodoEducativo:'',coruseId:'',carId:'',sede:'',dateLost:'',CodSol:''})
    ...
    const GetApiTeachersCourses = async () => {
        try {
            const TeacherCoursesData: any = await apiRecuperarAdelantar.listTeacherCourses(teacherCodeVal)
    ...
    const GetApiClassDate = async (getDate:string) => {
        try {
            const dataFormated = []
            const ClassDateData: any = await apiRecuperarAdelantar.listClassDate(TeacherCoursesSelected.ClaCodigo,teacherCodeVal,getDate)
    ...
    const PostApiScheduleSessions = async (dateValue:string,Hour:any) => {
        try {
            ...
            const ScheduleSessionsData: any = await apiRecuperarAdelantar.ScheduleSessions(dataSchedule.action,dataSchedule.classCode,dataSchedule.semesterCode,dataSchedule.date,dataSchedule.teacherCode)
    ...
    const PostTeacherAttendanceRecoverys = async (classCode:any,teacherCode:any,classDate:any,dateRequired:any,codeHour:any,nroHours:any,classRoomCode:any,user:any,host:any,hourIdProgInitial:any,hourIdProgFinal:any,type:any,path:any) => {
        try {
            setloading(true)
            const  dataSchedule = {
                    classCode,
            ...
            const ScheduleSessionsData: any = await apiRecuperarAdelantar.AttendanceRecoverys(dataSchedule.classCode,dataSchedule.teacherCode,dataSchedule.classDate,dataSchedule.dateRequired,dataSchedule.codeHour,dataSchedule.nroHours,dataSchedule.classRoomCode,dataSchedule.user,dataSchedule.host,dataSchedule.hourIdProgInitial,dataSchedule.hourIdProgFinal,dataSchedule.type,dataSchedule.path)
            setloading(false)
            ...
            case 4:
                {
                    if(fecha.indexOf('-') > -1){
                        ...
                        DateFormater = DateFormater[2] + "-" + DateFormater[1] + "-" + DateFormater[0] + "T00:00:00"
    ...
    const formatedDataRecovery= (obj: any, setstate: any) => {
        ...
        ...item,
        select: (
            <label className={styles.LinkStyle} onClick={(e) => CheckRB(e,item)}>
    ...
    const SendDate = (date:any) => {
        ...
        const result = formated[2] + "-" + formated[1] + "-" + formated[0] + "T" + "00:00:00"
        return result
    }
    ...
    const BTNSAVE = () => {
        ...
        const value:any = LaboratoriesList.find((x:any) => x.HorInicioDesc === SelectedHour)
        ...
        if(DateSelectedItem === "" || DateSelectedItem === "undefined/undefined/"){
            ViewMessage(8)
            valor = 1
        }
        ...
        if(valor === 0) 
        return PostTeacherAttendanceRecoverys(clase,teacherCodeVal,SendDate(fechaclase),SendDate(fecharequerida),hora,nrohoras,codAula,UserName,host,nHoraIdProgInicial,nHoraIdProgFin,tipo,path) 
    }
    ...
    const ValidaDateOnchangeText = (event: React.FormEvent<HTMLInputElement>) => {
        ...
        if(ClassDate.length > 0){
            if(Holyday.length > 0){
                ...
                const response = Holyday.find((x:any) => formatDate(x.date,3) === DateSelected)
  ...
  // validate
    ...
    const LlenarDDlAula = (DateToRecover:any) => {
        if(MissedClassDate !== "-- Seleccione --" && DateToRecover !== ""){
            ...
            if(TableSecondary.HorFin !== ""){
                ...
                DateToRecover = DateToRecover[2] + "-" + DateToRecover[1] + "-" + DateToRecover[0]   
                GetApiClasEnabled("",TeacherCoursesSelected.SedCodigo,DateToRecover,HorCodigo.HorCodigo,TableSecondary.NroHoras.toString())
            }
    ...
    const ObtenerHoraFin = async (Data:any,index:number,HourSelec?:string) => {
        if(Data.HorFin !== ""){
            let value:any
            if(index !== 0){
                value = await LaboratoriesList.find((x:any) => x.HorInicioDesc === SelectedHour)
            }else{
                value = await  LaboratoriesList.find((x:any) => x.HorInicioDesc === HourSelec)
            ...
            if(Laboratories.length > 0){
                const HorCodigo:any = await LaboratoriesList.find((x:any) => x.HorCodigo === response) 
        ...
        <div className={styles.contenido}>
            ...
            <div className={styles.alertaContent}>
                ...
                >
                    <b>Nota: </b>
                    <ul className="mb-0">
                        <li>Para reprogramar una sesión de clase, la solicitud debe ingresarse como mínimo <strong>24 horas</strong> antes de la fecha propuesta para adelanto o recuperación.</li>
                        <li>Las solicitudes de adelanto/recuperación se enviarán a los coordinadores de carrera, quienes deberán realizar la <strong>aprobación</strong> de las solicitudes antes del dictado de la sesión.</li>
                        <li>Para mayor información puede consultar el procedimiento vigente de <strong>Desarrollo de sesiones de clase</strong> ubicado en la carpeta de Mis Procesos UPN en la siguiente ruta:  
                            <b>
                            <Anchor
                            href=""
...

            {
                ViewRB === true ? (
                    <div className={styles.blockRadioButton}>
                        <RadioBtn type={'radio'} id={'recuperar'} name={'btnradio'} value={'Recuperar'} disabled={false} onclick={RB_ACTIVE_RETRIEVE} checked={BtnCheckedR}   readOnly/>
                        <RadioBtn type={'radio'} id={'adelantar'} name={'btnradio'} value={'Adelantar'} disabled={false} onclick={RB_ACTIVE_ADVANCE} checked={BtnCheckedA}  readOnly/>
                    </div>
                ) : null
            }

            {
                RBactiveViewPanel === true ? (
                    ...
                    <div className={styles.blocFormValidateRow}>
                        <ViewList texLabel={AlterTextView.FechaClase} optionSelect={ClassDate} onChange={OnchageValidateLst} consult={2}/>
                        {
                            ClassDate.length > 0 ? (
                                <div>
                                    <ViewInput texLabel={AlterTextView.FechaPropuesta} typeInput={'date'} nameInput={''} idInput={''} Onchange={ValidaDateOnchangeText} min={DateValidate === true ? DateMin() : ""} onClick={() => setDateValidate(true)} max={DateValidate ===true ? DateMax() : ""}/>
                                ...
                                <div>
                                     <div className={styles.viewInputConten}>
                                        <Label>
                                            {AlterTextView.FechaPropuesta}
                                        </Label> 
                                    <Input type={'text'} name={''} id={''} onchange={ValidaDateOnchangeText} placeholder=" " />
                                </div>
                                {
                                    ViewError === true ? (
                                        <div className={styles.ViewError}>
                    ...
                    <div className={styles.blocFormValidateRowTable}>
                        ...
                        {
                            (ViewTableData === false)  ? (
                                <ViewTable textLabel={AlterTextView.Horario} theadColums={[]} tbodyRows={[]}/>
                            ):
                            (ViewTableData && TableSecondary.HorFin === "") ?
                            (
                                <ViewTable textLabel={AlterTextView.Horario} theadColums={COLUMNS_SESION} tbodyRows={[]}/>   
                            ):(
                                <ViewTable textLabel={AlterTextView.Horario} theadColums={COLUMNS_SESION} tbodyRows={[convertTimeto24(TableSecondary.HorInicio),convertTimeto24(TableSecondary.HorFin),TableSecondary.NroHoras]} active={0} Onclick={SelectedTableSecondary}/>   
                            )
                        }
                    ...
                    <br />
                    <div className={styles.blocFormValidateRow}>
                        <ViewList texLabel={AlterTextView.HoraPropuesta} optionSelect={Laboratories} onChange={OnchangeValidateHoraPropuesta} index={0}/>
                        {
                            BtnSelected !== 0 ? (

                                 <ViewInput texLabel={AlterTextView.HoraPropuestaFin} typeInput={'text'} nameInput={''} idInput={''} value={convertTimeto24(viewCalculatedTime)} placeholder='' disabled={true} />
                                
                            ) : (
                                <Input style={{width:'50%'}} classname='md-3' type="text" disabled={true} name={''} id={''} placeholder=' '/>
...

                    {
                        ViewPanel === true ? (
                            <div>
                                <div className={styles.blocFormValidateRow}>
                                    <ViewList texLabel={'Aula:'} optionSelect={ClasEnabled} index={0} onChange={OnchangeAulaEdit} consult={2}/>

                                    {
                                        ClasEnabled.length > 0 ? (
                                            <ViewTexarea texLabel={'Caracteristicas del ambiente:'} placeholder=" " disabled={true}  onchange={() => setDesabledBTN({btnSave:false})}/>
                                        ) : (
                    ...
                    <div>
                        <b>
                        <RadioBtn type={'checkbox'} id={'default'} name={'check'} value={'Acepto los términos y condiciones correspondientes al proceso CONTROL DE RECUPERACIÓN Y ADELANTO DE CLASES.'} disabled={false} onclick={ActiveCheckedAcept} checked={BtnCheckedAcep} readOnly/>
                        </b>
                    </div>
